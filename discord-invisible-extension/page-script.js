(function () {
  ...
  if (window.__DISCORD_STEG__ && window.__DISCORD_STEG__.installed) {
    const existing = window.__DISCORD_STEG__;
    const scriptTag = document.currentScript;
    if (existing.setEnabled && scriptTag && scriptTag.dataset && scriptTag.dataset.enabled) {
      existing.setEnabled(scriptTag.dataset.enabled === "1");
    }
    return;
  }
  ...
  function getInitialEnabled() {
    const scriptTag = document.currentScript;
    if (scriptTag && scriptTag.dataset && scriptTag.dataset.enabled) {
      return scriptTag.dataset.enabled === "1";
    }
    return true;
  }
  ...
  window.addEventListener("message", function (event) {
    ...
    if (data.type === "set-key") setSharedKey(data.key);
    if (data.type === "fetch-response" && data.id) {
      const resolver = pendingFetches.get(data.id);
      if (!resolver) return;
      pendingFetches.delete(data.id);
      if (data.ok && data.data) {
        const bytes = base64DecodeBytes(data.data);
        if (bytes) {
          resolver(bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength));
          return;
        }
      }
      resolver(null);
  ...
  function fetchViaExtension(url) {
    return new Promise((resolve) => {
      const id = String(++fetchSeq) + "-" + Date.now();
  ...
  if (!state.sharedKey) state.sharedKey = SHARED_KEY;
  ...
  function createInvisibleCodec(alphabet, marker) {
    ...
    function decodeFromContent(content) {
      ...
      for (let offset = 0; offset < g; offset++) {
        ...
        try {
          const decoded = decodePayload(slice);
          if (decoded && decoded.startsWith(MAGIC)) {
  ...
  function getCryptoKeys() {
    ...
    if (!keyText) {
      if (!warnedMissingKey) {
        ...
        debugLog("Missing SHARED_KEY; encryption disabled");
      }
  ...
  function aesAddRoundKey(state, expandedKey, round) {
  ...
  function aesEncryptBlock(block, expandedKey) {
    ...
    aesSubBytes(state);
    aesShiftRows(state);
    aesAddRoundKey(state, expandedKey, AES_NR);
    return state;
  }
  ...
  function aesGcmEncrypt(plainBytes, keyState, ivBytes) {
    ...
    const y = ghash(hSubkey, cipherBytes);
  ...
  function gmFetchArrayBuffer(url) {
    ...
    if (typeof fetchViaExtension === "function") {
      return fetchViaExtension(url);
    }
    return Promise.resolve(null);
  }
  ...
  function ensureAttachmentStyles() {
    ...
    style.textContent =
      ...
      ".dhi-text-preview{margin:0;padding:0;background:transparent;white-space:pre-wrap;word-break:break-word;font:inherit;color:inherit;}" +
  ...
  function wrapMessageListener(listener) {
    const existing = messageListenerMap.get(listener);
